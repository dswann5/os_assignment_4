...
#ifndef TEST_PREFIX
#ifndef TEST_NAME
#error Define TEST_NAME or TEST_PREFIX before including test-tapish
#endif
...
#define TEST_FAIL()         do { printf(2, TEST_PREFIX "not ok\n"                          );         } while(0);
#define TEST_FINI()         do { printf(2, TEST_PREFIX "ok\n"                              );         } while(0);
...
/* Composites */
#define TEST_FINI_MSG(msg, ...) do { TEST_DIAG(msg, ##__VA_ARGS__); TEST_FINI(); } while(0);
#define TEST_FAIL_MSG(msg, ...) do { TEST_DIAG(msg, ##__VA_ARGS__); TEST_FAIL(); } while(0);

/* Conditionals */
#define TEST_FAIL_IF(cond, msg, ...) if(cond) { TEST_FAIL_MSG(msg, ##__VA_ARGS__); }
...
#define TEST_FAIL_IF_THEN(cond, then, msg, ...) if(cond) { TEST_FAIL_MSG(msg, ##__VA_ARGS__); then; }
